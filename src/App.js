...
const App = () => {
  // keep track of dark/light theme and sync with html element
  ...
  // set CSS variables when theme changes (no dark: variants used)
  useEffect(() => {
    try { localStorage.setItem('theme', theme); } catch {}
    // mirror the same variable sets used in public/index.html
    var light = {
    ...
    var dark = {
      ...
      '--color-border': '#111827',
      '--color-card': '#12121a',
      '--color-card-text': '#eef2ff',
      '--color-btn': '#111827',
      '--color-btn-text': '#eef2ff'
    };
  ...
  return (
    <div className="min-h-screen bg-primary text-text font-sans selection:bg-accent">
      
      {/* Navigation with theme toggle */}
      <nav className="sticky top-0 z-50 bg-surface border-b border-border backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-500/20">
              <Shield className="w-5 h-5 text-cardText" />
            </div>
            <div className="flex flex-col">
              <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-cardText to-subtext leading-tight">
                CREDIT & LOANS
              </span>
              <span className="text-[10px] text-subtext font-medium tracking-wide hidden sm:block">
          ...
          <div className="flex items-center gap-4">
            <div className="hidden sm:block">
               <span className="text-sm font-medium text-subtext tracking-widest uppercase">Credit Card Portal</span>
            </div>
            <button onClick={toggleTheme} className="p-2 rounded-full bg-btn text-btnText hover:opacity-90">
              {theme === 'dark' ? (
                <Sun className="w-5 h-5 text-yellow-400" />
              ) : (
                <Moon className="w-5 h-5 text-cardText" />
              )}
            </button>
          </div>
        </div>
      ...
      <section className="relative pt-16 pb-12 px-6 overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center">
          <h1 className="text-4xl md:text-6xl font-extrabold text-text mb-6 tracking-tight">
            Discover your next <br className="hidden md:block" />
            <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400">
              premium credit card.
            </span>
          </h1>
          <p className="text-lg text-subtext mb-10 max-w-2xl mx-auto">
              ...
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                  selectedCategory === category
                    ? 'bg-card text-cardText shadow-[0_0_20px_rgba(0,0,0,0.06)]'
                    : 'bg-surface text-subtext hover:opacity-95 border border-border'
                }`}
              >
                {category}
              </button>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          {filteredCards.map((card) => (
            <div 
              key={card.id} 
              className={`bg-card border ${card.popular ? 'border-indigo-500/50 shadow-[0_0_30px_rgba(99,102,241,0.1)]' : 'border-border'} rounded-3xl p-6 md:p-8 flex flex-col relative group hover:-translate-y-2 transition-transform duration-500`}
            >
              {card.popular && (
              ...
              <div className={`relative h-56 rounded-2xl p-6 flex flex-col justify-between overflow-hidden shadow-2xl mb-8 ${card.border || 'border border-white/10'}`}>
                ...
                <div className="relative z-10 flex justify-between items-start">
                  <div className="text-cardText font-bold tracking-widest text-lg">
                    {card.brand}
                  </div>
                  <svg className="w-8 h-8 text-cardText" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z" />
                  </svg>
                </div>

                <div className="relative z-10 font-mono text-xl tracking-[0.15em] text-cardText mt-4">
                  {card.cardNumber}
                </div>

                <div className="relative z-10 mt-auto flex justify-between items-end">
                  <div>
                    <p className="text-[10px] text-subtext uppercase tracking-widest mb-1">CLIENT</p>
                    <p className="text-sm font-semibold text-cardText tracking-wider">VALUED MEMBER</p>
                  </div>
                  {/* Fake Chip */}
              ...
              <div className="flex-1 flex flex-col">
                <div className="mb-6">
                  <h3 className="text-2xl font-bold text-cardText mb-1">{card.name}</h3>
                  <div className="inline-block px-3 py-1 bg-surface rounded-md text-xs text-indigo-300 font-medium mb-2 border border-border">
                    {card.category}
                  </div>
                </div>

                <div className="mb-6 pb-6 border-b border-border">
                  <p className="text-xs text-subtext uppercase tracking-wider mb-1">Annual Fee</p>
                  <p className="text-lg font-semibold text-cardText">{card.annualFee}</p>
                </div>
                ...
                <ul className="space-y-4 mb-8 flex-1">
                  {card.features.map((feature, idx) => (
                    <li key={idx} className="flex items-start gap-3">
                      ...
                      <span className="text-sm text-subtext leading-relaxed">{feature}</span>
                    </li>
                  ))}
                </ul>
                ...
                <button 
                  ...
                  className={`w-full py-4 rounded-xl font-semibold flex items-center justify-center gap-2 transition-all duration-300 ${
                    card.popular 
                      ...
                      : 'bg-btn text-btnText hover:opacity-95 border border-border'
                  }`}
                >
      ...
      {/* Contact Section */}
      <section className="bg-surface border-t border-border py-16 px-6 relative overflow-hidden">
        ...
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold text-cardText mb-3">CREDIT & LOANS</h2>
          <p className="text-lg text-subtext mb-10">Your trusted partner for financial freedom.</p>
          
          <div className="inline-flex flex-col items-center bg-card border border-border rounded-3xl p-8 shadow-2xl relative group">
            ...
            <div className="relative flex flex-col items-center">
              <div className="w-20 h-20 rounded-full bg-gradient-to-br from-green-400 to-emerald-600 flex items-center justify-center mb-5 shadow-lg shadow-green-500/25">
                <svg className="w-10 h-10 text-cardText" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
              ...
              <h3 className="text-2xl font-bold text-cardText mb-2">Manish Malviya</h3>
